...
- Owned areas: Postgram TypeScript/Node.js backend services, REST API, MCP SSE transport, CLI and admin CLI, database migrations, enrichment/search/graph services, tests, docs, UI, browser extension packages, and repository configuration.
- Out-of-scope areas: production user data, deployed infrastructure state, external account configuration, and third-party service behavior unless a WDD epic explicitly includes an adapter or operational change.
- External systems treated as adapters: PostgreSQL with pgvector, Docker Compose runtime, MCP clients, local markdown repositories, OpenAI, Anthropic, Ollama, browser extension platforms, and any external automation or tracker integrated with Postgram.
...
- Project type: TypeScript Node.js application with npm workspaces.
- Package manager: npm (`package-lock.json` is present).
- Runtime requirement: Node.js >=22.
- Native verification commands: `npm run typecheck`, `npm test`, `npm run lint`, `npm run build`, and focused Vitest or workspace checks as appropriate for the touched area.
- Primary architecture boundaries: service layer, transport layer, CLI/admin CLI, persistence/migrations, background enrichment, UI, browser extension packages, and integration tests.
...
- Postgram changes should choose the smallest meaningful command set from the
  native verification commands listed in Project Context, scaling up to full
  typecheck/test/lint/build when the blast radius is broad.
...
- Should WDD branch names use `codex/epic/...` and `codex/task/...` for this repository, or should issue-style branches such as `codex/issue-...` remain the norm?
- Which configured model aliases should be recorded for planning, implementation, review, feedback fixes, validation, and PR descriptions?
- Should the default WDD profile remain `standard`, or should Postgram prefer `micro`/`lite` for routine issue-sized changes?
- Are there additional ownership boundaries, deployment environments, or external trackers that should be named before the first epic?
