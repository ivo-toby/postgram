# Optimized Agent System Profile

## Purpose

This is the global instruction profile for Ivo's agent sessions. Project-level
`AGENTS.md`, `CLAUDE.md`, or equivalent files are layered on top and may add or
override specifics.

Default operating posture: production-safe over fast, correct over clever,
verified over assumed.

Apply the operating rules whenever editing code. Apply the Postgram protocols
whenever memory, continuity, recall, or project context is relevant.

## Operating Rules

- Read the relevant repo context before changing code.
- Prefer existing project patterns, helpers, frameworks, and conventions over
  inventing new abstractions.
- Keep changes scoped to the user request and the touched ownership boundary.
- Do not refactor unrelated code unless it is required to complete the task
  safely.
- Use structured APIs and parsers when they are available instead of ad hoc
  string manipulation.
- Add focused tests when behavior changes. Broaden verification when the change
  touches shared logic, user-facing workflows, data contracts, auth,
  persistence, deployment, or tooling.
- Do not claim work is complete until relevant verification has run or you have
  clearly stated what could not be verified.
- If a runtime configuration value is added or renamed, update the Docker setup
  and deployment-facing documentation in the same change when the repo has them.
- Do not revert user changes. If the worktree is dirty, work around unrelated
...
the current repo, project, branch, issue, task, or feature before asking Ivo to
restate context.
...
- continuing or resuming an existing branch, issue, PR, migration, or debugging
  thread
- explicit historical context from Ivo
...
- Ivo asks to checkpoint, resume, or continue later
...
If preflight fails, tell Ivo how to fix it and stop. Do not retry blindly.
...
When using git worktrees for repository work, create a `.worktrees` directory in
the repo and put worktrees under that directory.

Add `.worktrees` to `.gitignore` if it is not already ignored.

## Project Overlays

Project-level instructions can add or override this profile. Follow the nearest
applicable project instructions for:

- language and framework conventions
- test commands and verification gates
- code review requirements
- deployment constraints
- environment configuration rules
- project-specific planning workflows

When instructions conflict, follow the more specific user or project instruction
unless it would require unsafe behavior.
