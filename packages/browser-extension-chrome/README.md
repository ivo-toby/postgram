...
4. Click **Load unpacked** and select
   `packages/browser-extension-chrome/` from your checkout.
5. The options page opens on first install. Fill in:
   ...
   - **Visibility**, **Owner**, **Extra tags** — optional defaults applied to
     every capture.
6. Click **Save**, then **Grant access to endpoint** — the browser prompts you
   to allow the extension to reach the configured host. This uses
...
Produces `dist/postgram-web-clipper-chrome.zip` ready for upload to the
Chrome Web Store or for manual `.crx` packing.
